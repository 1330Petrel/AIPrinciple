...
class NQLearning:
    ...
    def __init__(
        ...
        n_states: 状态数量
        n_actions: 动作数量
        n_step: n-step Q-learning 中的 n 步
        ...
        # N-step buffer: 存储 (s, a, r, s_)
        self.memory = deque()

        # 预计算 gamma 的次方: [1, gamma, gamma^2, ...]
        self._gamma_pow = np.array([self.gamma**i for i in range(n_step + 1)])

    def learn(self, s: int, a: int, r: float, s_: int, done: bool) -> None:
        """N-step Q-learning 更新
        ...
        G = r_0 + gamma * r_1 + ... + gamma^(k-1) * r_{k-1} + gamma^k * max_Q(s_final)
        其中 k = n_step 或直到终止状态
        """
        # 1. 存 transition
...
class Agent:
    def __init__(
        ...
        logger.info(f"Usr config: {config_dict}")
...
def train(env: Maze, agent: Agent, visualize: bool = Config.RENDER_TRAINING) -> None:
    ...
    try:
        ...
        Utils.plot_training(
            epsilon_rewards, episode_steps, episode_wins, episode_epsilons
        )
